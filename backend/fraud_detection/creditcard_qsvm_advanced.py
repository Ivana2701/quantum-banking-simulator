...
from qiskit.circuit.library import ZZFeatureMap, PauliFeatureMap, TwoLocal, EfficientSU2
...
class AdvancedCreditCardQSVM:
    def __init__(self, n_qubits=10, feature_map_type='efficient_su2', reps=5, entanglement='full'):
        """
        Initialize Advanced Quantum SVM for Credit Card Fraud Detection
        
        Args:
            n_qubits: Number of qubits (increased to 10 for maximum quantum advantage)
            feature_map_type: 'efficient_su2', 'zz', 'pauli', or 'two_local'
    ...
    def create_advanced_quantum_feature_map(self, feature_dim):
        """
        Create advanced quantum feature map with maximum qubits
        
        Args:
            feature_dim: Dimension of input features
        """
        if self.feature_map_type == 'efficient_su2':
            # Most sophisticated feature map
            self.feature_map = EfficientSU2(
    ...
    def train_advanced_quantum_svm(self, X_train, y_train):
        """
        Train Advanced Quantum SVM with maximum qubits
        ...
        feature_map = self.create_advanced_quantum_feature_map(X_train.shape[1])
        ...
        print("Training Advanced Quantum SVM...")
        print(f"Using {self.n_qubits} qubits with {self.reps} repetitions")
        self.qsvc.fit(X_train, y_train)
        print("Training completed!")
        
        return self.qsvc
    
    def evaluate_model(self, X_test, y_test, model_name="Advanced Quantum SVM"):
        """
    ...
    def train_and_evaluate(self, sample_size=10000):
        ...
        # Train model
        self.train_advanced_quantum_svm(X_train, y_train)
        ...
        metrics_path = os.path.join(script_dir, "metrics_data", "creditcard_qsvm_advanced.json")
        self.save_metrics(metrics, metrics_path)
        
        return metrics

def run_advanced_creditcard_qsvm():
    """Main function to run the advanced credit card QSVM"""
    print("Starting Advanced Credit Card Fraud Detection with Quantum SVM")
    print("="*70)
    
    # Create model with maximum qubits
    qsvm = AdvancedCreditCardQSVM(
        n_qubits=10,              # Maximum qubits for quantum advantage
        feature_map_type='efficient_su2', # Most sophisticated feature map
        reps=5,                   # Maximum repetitions
    ...
    print("\n✅ Advanced training and evaluation completed!")
    return metrics

if __name__ == "__main__":
    run_advanced_creditcard_qsvm() 