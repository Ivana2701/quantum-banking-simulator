...
def preprocess_features(X, y, n_qubits=4, test_size=0.2):
    scaler = RobustScaler()
...
def train_and_evaluate_ibmq_vqc(sample_size=1000, n_qubits=4):
    X, y = load_creditcard_data(sample_size=sample_size)
    X_train, X_test, y_train, y_test = preprocess_features(X, y, n_qubits=n_qubits)
    feature_map = PauliFeatureMap(feature_dimension=n_qubits, reps=2, paulis=['Z','X','ZZ'])
    ...
    print("Training VQC on IBMQ backend...")
    vqc.fit(X_train, y_train)
    print("Training completed!")
    y_pred = vqc.predict(X_test)
    try:
        y_score = vqc.predict_proba(X_test)[:, 1]
    except:
    ...
    metrics_path = os.path.join(script_dir, "metrics_data", "creditcard_vqc_ibmq.json")
...
if __name__ == "__main__":
    train_and_evaluate_ibmq_vqc(sample_size=1000, n_qubits=4) 